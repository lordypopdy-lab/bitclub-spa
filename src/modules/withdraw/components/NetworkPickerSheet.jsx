        ...
        {(a?.networks || []).map((n) => {
          ...
          return (
            <button key={n.id} onClick={() => { onSelect(n.id); onClose(); }}
              style={{ width: "100%", textAlign: "left", padding: 14, marginBottom: 10, borderRadius: 12,
              ...
              <div style={{ color: "#9aa0a8", fontSize: 12, marginTop: 4 }}>Fee: {n.fee} {symbol} · Min {n.min} · Crediting {n.eta}</div>
            </button>
          );
        })}
      </div>
